...
$(function(){
    //chamada da função
    preco(parametros_pesquisa);
    
    function preco (itens){

        $(".refresh-loader").show();
        var qtd = itens.quantidade

        var valor = camisetas[itens.cor][itens.gola][itens.estampa].preco_unit;


        var imagem = "src/" + camisetas[itens.cor][itens.gola][itens.estampa].foto;

        var total = qtd*valor;


        if(itens.qualidade == "q190"){

            total *= 1.12;
        }
        if(itens.embalagem =="unitaria"){
            total += qtd * 0.15;
        }
